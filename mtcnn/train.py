...
class Trainer:
    ...
    def train(self):
        ...
        faceDataloader = DataLoader(faceDataset, batch_size=256, shuffle=True, num_workers=4, drop_last=True)
        j = 0
        while True:
            for i, (img_data, cond_target, face_offset_target) in enumerate(faceDataloader):
                if self.isCuda:
                ...
                # 网络输出  CWH
                cond_output_, offset_face_output_ = self.net(img_data)
                ...
                # 总损失
                loss = cond_loss + offset_face_loss
                self.opt.zero_grad()
                loss.backward()
                self.opt.step()
