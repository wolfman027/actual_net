...
class Detector:
    ...
    def __pnet_detector(self, image):
        ...
        while min_side_len > 12:  # 直到缩放到小于等于12时停止
            ...
            _cond, _offset_face = self.pnet(img_data)  # 返回多个置信度和偏移量
            # [245, 203]：特征图的尺寸：H,W
            ...
            boxes_ = self.__box(idxs, cond, offset_face, scale)
            if len(boxes) > 0:
    ...
    # p网络池化步长为2
    def __box(self, idxs, cond, offset_face, scale, stride=2, side_len=12):
    ...
    def __rnet_detect(self, image, pnet_boxes):
        ...
        return utils.nms(np.array(boxes_aaaaaaa), r_nms)  # 原r_nms为0.5（0.5要往小调），上面的0.6要往大调;小于0.5的框被保留下来
    ...
    def __onet_detect(self, image, rnet_boxes):
        ...
        boxes_ = np.array([x1, y1, x2, y2, cls], dtype=np.float64)
        boxes_aaaaaaa = np.swapaxes(boxes_, 1, 0)
        return utils.nms(np.array(boxes_aaaaaaa), o_nms, isMin=True)  # 用最小面积的IOU；原o_nms(IOU)为小于0.7的框被保留下来
